...
def button_click(char):
    ...
    text_display.delete(1.0, END)  # Clear the Text widget
    text_display.insert(END, calc_operator)  # Insert updated calc_operator
...
def fact_func():
    ...
    text_display.insert(END, result)
...
def button_equal():
    ...
    try:
        ...
        text_display.delete(1.0, END)
        text_display.insert(END, temp_op)
        calc_operator = temp_op
    except:
        text_display.delete(1.0, END)
        text_display.insert(END, "ERROR")
...
tk_calc.title("Scientific Calculator By Ali Raza")

calc_operator = ""
text_display = Text(tk_calc, font=('helvetica', 30,), height=2, width=15, bd=14, bg='#FFFFFF', fg='#000000')
text_display.grid(columnspan=5, padx=6, pady=10)
text_display.insert(END, "")  # Initially empty


button_params = {'bd':5, 'fg':'#FFF', 'bg':'#BBBBBB', 'font':('helvetica', 12,), 'width': 3, 'height': 1}
button_params_main = {'bd':5, 'fg':'#000', 'bg':'#DEB004', 'font':('helvetica', 15,)}
...
# Powers of 10
tens_powers = Button(tk_calc, button_params, text='10^x', font=('helvetica', 15, 'bold'),
...
delete_one = Button(tk_calc, bd=5, fg='#FFFFFF', font=('helvetica', 20, 'bold'),
              text='DEL', command=button_delete, bg='#A50000').grid(row=6, column=3, sticky="nsew")
delete_all = Button(tk_calc, bd=5, fg='#FFFFFF', font=('helvetica', 20, 'bold'),
              text='AC', command=button_clear_all, bg='#A50000').grid(row=6, column=4, sticky="nsew")
...
button_multiply = Button(tk_calc, button_params_main, text='x',
                         command=lambda:button_click('*')).grid(row=7, column=3, sticky="nsew")
button_divide = Button(tk_calc, button_params_main, text='÷',
...
button_add = Button(tk_calc, button_params_main, text='+',
                    command=lambda:button_click('+')).grid(row=8, column=3, sticky="nsew")
button_subtract = Button(tk_calc, button_params_main, text='-',
                         command=lambda:button_click('-')).grid(row=8, column=4, sticky="nsew")

#--9th row--
button_0 = Button(tk_calc, button_params_main, text='0',
                  command=lambda:button_click('0')).grid(row=9, column=0, columnspan=2, sticky="nsew")
button_decimal = Button(tk_calc, button_params_main, text='.',
                        command=lambda:button_click('.')).grid(row=9, column=2, sticky="nsew")
button_equal = Button(tk_calc, bd=5, fg='#FFFFFF', font=('arial', 20, 'bold'),
                      text='=', command=button_equal, bg='#A50000').grid(row=9, column=3, columnspan=2, sticky="nsew")

'''
Grid Configuration
'''
for i in range(10):  # Configure 10 rows
    tk_calc.grid_rowconfigure(i, weight=1)
for j in range(5):  # Configure 5 columns
